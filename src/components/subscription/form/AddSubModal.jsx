...
export default function AddSubModal({
  ...
  const [defaultPayment] = useLocalStorage("defaultPayment", "");
  const [form, setForm] = useState(() => ({ ...EMPTY_SUBSCRIPTION_FORM, paymentMethod: defaultPayment }));
  ...
  const displayName = trimmedName || "新增訂閱";
  const avatarInitial = trimmedName ? trimmedName.charAt(0).toUpperCase() : "N";
  ...
  function handleSubmit(e) {
    ...
    onAdd(buildNewSubscriptionPayload(form));
    onClose();
  }

  function resetForm() {
    setForm({ ...EMPTY_SUBSCRIPTION_FORM, paymentMethod: defaultPayment });
    setErrors({});
  }
    ...
    <>
      <Dialog open={true} onOpenChange={onClose}>
        <DialogContent
          ...
          showClose={false}
          title="新增訂閱"
          description="新增一筆訂閱服務，包含名稱、金額、扣款週期、付款方式與分帳設定。"
        >
          <div
              ...
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                ...
                <div className="flex items-center justify-end gap-3">
                  ...
                  <Button
                    ...
                    form="add-sub-form"
                    variant="default"
                    className="min-w-36 rounded-2xl shadow-[0_12px_28px_rgba(29,53,87,0.28)]"
                  >
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24">
